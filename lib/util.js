...
module.exports.generateCacheDir = function(cacheDirName){
    var rand      = Math.floor(Math.random() * 1000000),
        cachePath = path.join(util.getAnyProxyHome(),"./" + cacheDirName);

    fs.mkdirSync(cachePath);
    return cachePath;
}

module.exports.clearCacheDir = function(cacheDirName, cb){
    var home  = util.getAnyProxyHome(),
        isWin = /^win/.test(process.platform);

    // var dirNameWildCard = CACHE_DIR_PREFIX + "*";
    if(isWin){
        exec("for /D %f in (" + cacheDirName + ") do rmdir %f /s /q",{cwd : home},cb);
    }else{
        exec("rm -rf " + cacheDirName + "",{cwd : home},cb);
    }
}
